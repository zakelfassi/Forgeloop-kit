...
defmodule ForgeloopV2.Config do
  ...
  defstruct [
    ...
    :deploy_cmd,
    :deploy_smoke_cmd,
    :post_deploy_ingest_logs,
    :post_deploy_observe_seconds,
    :ingest_logs_cmd,
    :ingest_logs_file,
    :ingest_logs_tail,
    :default_branch,
  ...
  def load(opts \\ []) do
    with {:ok, %{repo_root: repo_root, forgeloop_root: forgeloop_root}} <- ForgeloopV2.RepoPaths.resolve(opts) do
      ...
      config = %__MODULE__{
        ...
        deploy_pre_cmd: optional_string(opts[:deploy_pre_cmd], "FORGELOOP_DEPLOY_PRE_CMD", shell_env),
        deploy_cmd: optional_string(opts[:deploy_cmd], "FORGELOOP_DEPLOY_CMD", shell_env),
        deploy_smoke_cmd: optional_string(opts[:deploy_smoke_cmd], "FORGELOOP_DEPLOY_SMOKE_CMD", shell_env),
        post_deploy_ingest_logs:
          truthy?(opts[:post_deploy_ingest_logs], env_value("FORGELOOP_POST_DEPLOY_INGEST_LOGS", shell_env), false),
        post_deploy_observe_seconds:
          non_negative_int(opts[:post_deploy_observe_seconds], "FORGELOOP_POST_DEPLOY_OBSERVE_SECONDS", 0, shell_env),
        ingest_logs_cmd: optional_string(opts[:ingest_logs_cmd], "FORGELOOP_INGEST_LOGS_CMD", shell_env),
        ingest_logs_file: optional_string(opts[:ingest_logs_file], "FORGELOOP_INGEST_LOGS_FILE", shell_env),
        ingest_logs_tail: positive_int(opts[:ingest_logs_tail], "FORGELOOP_INGEST_LOGS_TAIL", 400, shell_env),
        ...
        max_daily_iterations: non_negative_int(opts[:max_daily_iterations], "FORGELOOP_MAX_DAILY_ITERATIONS", 200, shell_env),
        max_session_iterations: non_negative_int(opts[:max_session_iterations], "FORGELOOP_MAX_SESSION_ITERATIONS", 100, shell_env),
        max_stall_cycles: non_negative_int(opts[:max_stall_cycles], "FORGELOOP_MAX_STALL_CYCLES", 3, shell_env),
        max_read_slots: positive_int(opts[:max_read_slots], "FORGELOOP_MAX_READ_SLOTS", 3, shell_env),
        max_write_slots: positive_int(opts[:max_write_slots], "FORGELOOP_MAX_WRITE_SLOTS", 1, shell_env),
