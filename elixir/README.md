...
The next integration slice is still ahead of us: bash-daemon / wrapper convergence onto the babysitter path, stronger ownership semantics if worktree-aware claims ever land, native graph execution beyond the current external workflow runner path, richer workflow outcome/history projection, and workflow-aware daemon scheduling on top of the current OpenClaw/UI/service seams.
