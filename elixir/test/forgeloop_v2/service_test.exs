defmodule ForgeloopV2.ServiceTest do
  ...
  test "overview exposes runtime, backlog, questions, escalations, events, workflows, and babysitter state" do
    repo =
      create_repo_fixture!(
    ...
    config = config_for!(repo.repo_root, service_port: 0)
    ...
    assert length(payload["data"]["backlog"]["items"]) == 1
  ...
  test "babysitter endpoints serialize manual runs and allow stop through the loopback service" do
    ...
    config = config_for!(repo.repo_root, service_port: 0, shell_driver_enabled: true, babysitter_shutdown_grace_ms: 50)
    {:ok, pid, base_url} = start_service!(config)
    on_exit(fn -> Process.exit(pid, :shutdown) end)
  ...
  defp decode_response(response) do
    ...
    [_headers, body] = String.split(rest, "\r\n\r\n", parts: 2)
    %{status: String.to_integer(status), body: Jason.decode!(body)}
  end
end
