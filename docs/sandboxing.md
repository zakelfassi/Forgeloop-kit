...
Important current limits remain open: bash-daemon / wrapper convergence onto the babysitter path, broader workflow orchestration beyond the current one-shot Elixir-daemon `[WORKFLOW]` request, and any worktree-aware ownership-claim semantics.
