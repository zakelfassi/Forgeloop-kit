...
- the bounded multi-slot coordinator now exists as an experimental Elixir-only extension: checklist `plan` and workflow `preflight` can run in parallel slot-scoped disposable worktrees, and one serialized write slot can run checklist `build` or workflow `run` while repo-root runtime-state summarizes the coordinator with `mode: "slots"` / `transition: "coordinating"`
