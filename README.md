...
The workflow lane is intentionally narrow in this slice: still runner-backed, still mapped onto the same runtime-state + escalation contract, and still manual-first. Workflow `preflight` / `run` actions flow through the managed Elixir babysitter + disposable-worktree path, the loopback service/HUD/OpenClaw seam exposes the same workflow control/status surface, and the experimental Elixir daemon can now honor one explicit `[WORKFLOW]` marker by launching a single configured workflow target through that same managed path. The public `./forgeloop.sh daemon` wrapper still follows the bash path, and richer workflow scheduling/history remains deferred. Elixir also keeps a read-only workflow catalog/artifact view plus a repo-local tracker projection that maps canonical backlog items and workflow packs into `Tracker.Issue` structs without mutating external trackers yet.
